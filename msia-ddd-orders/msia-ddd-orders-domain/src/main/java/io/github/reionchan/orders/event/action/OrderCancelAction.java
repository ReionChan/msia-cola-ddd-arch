 ...
 **/
@Slf4j
public class OrderCancelAction implements Action<OrderStatus, OrderEventType, OrderEventContext> {

    @Override
    public void execute(OrderStatus from, OrderStatus to, OrderEventType event, OrderEventContext context) {
        log.debug("orderId {} has been canceled!", context.getAggregate().getOrderId());
