 ...
 **/
@Slf4j
public class OrderPayConfirmAction implements Action<OrderStatus, OrderEventType, OrderEventContext> {

    @Override
    public void execute(OrderStatus from, OrderStatus to, OrderEventType event, OrderEventContext context) {
        log.debug("orderId {} has been paid and confirmed!", context.getAggregate().getOrderId());
